...
def lootcomment(name, entry):
    ...
    def parse_function(item):
        match flat_function(item):
            case 'furnace_smelt':
            ...
            case 'set_enchantments':
                enchantments = item.get('enchantments', None)
                if not enchantments:
                    pass
                else:
            ...
            case 'set_random_potion':
                comment.append('random potion: '+ item['options'])
            
            case 'set_nbt':
                from nbtlib import parse_nbt
                j = parse_nbt(item['tag']).unpack(json=True)
                if 'Potion' in j:
    ...
    def parse_condition(item):
        ...
        match condition_type:
            ...
            case 'random_chance':
                comment.append('random chance: '+mcrange(name, item['chance'])+'%')
            case 'random_chance_with_looting':
                unenchanted_chance = no_end_0(item['chance'])+'%'
                            ...
                            match type_name:
                                ...
                                case 'villager/variant':
                                    if isinstance(predicate, str):
                                        predicate = [predicate]
                                    predicate = '/'.join(flatering(x) for x in predicate)
                                    comment.append(f'is {predicate} variant')
                                case 'predicates':
                                    for k,v in predicate.items():
...
def listing_loot_tables(temp):
    ...
    if entries:
        write_lines(os.path.join(temp, 'lists', dirname+'.txt'), sorted(entries) + sorted(tags))
    if blocks:
        write_lines(os.path.join(temp, 'lists', dirname+'.blocks.txt'), sorted(blocks) + sorted(tags_blocks))
...
def listing_timelines(temp):
    ...
    lst_namespace, _ = get_sub_folders_assets(temp)
    for ns in lst_namespace:
        for dpn, dp in get_datapack_paths(temp):
            dir = os.path.join(temp, dp, 'data', ns, 'timeline')
            for f in glob.iglob('**/*.json', root_dir=dir, recursive=True):
...
def listing_villager_trades(temp):
    ...
    def slot(tname, item, is_book=None):
        ...
        components = []
        for k,v in item.get('components', {}).items():
            match flatering(k):
    ...
    for ns in lst_namespace:
        for dpn, dp in get_datapack_paths(temp):
            dir = os.path.join(temp, dp, 'data', ns, 'villager_trade')
            for f in glob.iglob('**/*.json', root_dir=dir, recursive=True):
                data = read_json(os.path.join(dir, f))
                name = filename(f)
                lines = []
